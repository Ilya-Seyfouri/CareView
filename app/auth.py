...
from app.database import carers, managers, familys, verify_password,admins
...
def get_user(email: str):
    if email in carers:
        ...
        return {"user":admins[email], "user-type": "admin"}
...
async def get_current_carer(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if carer is None:
        raise invalid
    if carer["user-type"] not in ["carer", "manager"]:
        raise HTTPException(
...
async def get_current_family(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if family is None:
        raise invalid
    if family["user-type"] not in ["family-member", "manager"]:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
async def get_current_admin(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    admin = get_user(email=email)  # get carer using token ID
    if admin is None:
        raise invalid
